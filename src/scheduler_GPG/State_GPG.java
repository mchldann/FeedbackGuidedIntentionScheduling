...
public class State_GPG {
    	...
    	HashSet<TreeNode> candidateActions, HashSet<String> blockedPlans, TreeNode lastActionApplied)
    {
        ...
        this.pass_repetitions_exceeded = pass_repetitions_exceeded;
        this.age = age;
        this.actionHistory = new ArrayList<TreeNode>(actionHistory);
    ...
    public void applyAction(TreeNode action)
    {
        ...
        {
        	if (intention_completion_times[i] == -1 && isIntentionComplete(i))
            {
        		intention_completion_times[i] = age;
            }
        }
    }
    ...
    public ArrayList<TreeNode> getAvailableActions(boolean[] intentionAvailable)
    {
    	...
    	if ((result.size() == 0)
    			|| Enums.ALLOW_UNFORCED_PASS && (lastActionApplied == null || lastActionApplied instanceof ActionNode))
    	{	
    		result.add(null);
    	}
