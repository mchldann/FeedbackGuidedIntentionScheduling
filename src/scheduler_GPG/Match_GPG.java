...
public class Match_GPG
{
    ...
    private Match_Info run(boolean verbose, boolean write_results, boolean cache_results, boolean mirror_match, boolean[] visibility_mask)
    {
    	...
    	if (currentState.consecutive_passes >= Enums.MAX_CONSECUTIVE_PASSES)
    	{
        	...
        	currentState.game_over = true;
        	currentState.pass_repetitions_exceeded = true;
    	}
        ...
        for (int i = 0; i < numAgents; i++)
        {
        	...
        	if (schedulers[i] instanceof MCTS_Scheduler_GPG)
        	{
        		((MCTS_Scheduler_GPG)schedulers[i]).rollout_schedulers = null;
        	}
    ...
    public void writeResults(State_GPG s, long total_match_time, boolean cache_results)
    {
		try
		{
	        ...
	        if (firstWrite)
	        {
	        	...
	        	out.print("MatchName,ForestName,MatchTimeMillis,EndedByRepetition");
		        for (int i = 0; i < numAgents; i++)
		        {
            ...
            str.append("," + s.pass_repetitions_exceeded);
		...
		try
		{
			String csv_file = Log.getLogDir() + "/match_results_for_python.csv";
			File f = new File(csv_file);
			boolean firstWrite = !f.exists();
	        ...
	        if (firstWrite)
	        {
	        	...
	        	//out.print("MatchName,ForestName,AssumedPoliteness,MatchTimeMillis");
	        	out.print("MatchName,ForestName,MatchTimeMillis,EndedByRepetition");
		        for (int i = 0; i < s.intentions.size(); i++)
		        {
		        	out.print(",I" + i);
		        }
            ...
            for (int i = 0; i < s.intentions.size(); i++)
	        {
	        	str.append("," + (s.isIntentionComplete(i) ? 1 : 0));
	        }
	        ...
	        if (Main.oracle.uses_time)
            {
	            for (int i = 0; i < s.intentions.size(); i++)
		        {
		        	str.append("," + (Main.getCompletionTimeRepresentation(s.intention_completion_times[i])));
		        }
            }
